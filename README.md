# vibecode-security

Before you put an app online, this checks it for security problems and gives you a clear answer: **safe to launch**, or **not yet — and here's what to fix**. It talks in plain language. No tech background needed.

---

## Step 1 — Install it (you do this once)

In Claude, copy the **first** line below, paste it in, and press Enter. Then do the same with the **second** line.

```
/plugin marketplace add https://github.com/constellation-academy/vibecode-security.git
```

```
/plugin install vibecode-security@constellation-academy-agent-skills
```

Now close Claude and open it again. That's it — you don't need a GitHub account or any setup.

> On the **desktop app** you can also do the second part with your mouse: click the **+** next to the text box → **Plugins** → **Add plugin**, then click **vibecode-security**.

---

## Step 2 — Use it

Open your project in Claude and type:

> **check my app**

That's the whole thing. Claude looks through your app and tells you whether it's safe to go live — or exactly what to fix first. (You can also say it in German: **"kann ich das veröffentlichen?"**)

When Claude asks permission to run the check, click **Allow**. That's normal and needed.

---

## Step 3 — Keep it fresh (optional, but a good idea)

The security checks get better over time. To always get the newest ones, do this once:

1. Type **/plugin** and press Enter
2. Go to **Marketplaces**
3. Pick **constellation-academy-agent-skills**
4. Choose **Enable auto-update**

Done. From now on it updates itself — you never have to think about it again.

---

## Made by

[Sascha Ringert](https://constellation-academy.com) · [Constellation Academy](https://constellation-academy.com)
